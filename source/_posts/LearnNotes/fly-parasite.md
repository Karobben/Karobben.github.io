...
Transcriptional silencing of mobile elements could be acheived via the ==PIWI-interacting RNA (piRNA) pathway==. piRNA which contains target sequence paired with PIWI fanuky proteins *Piwi*, *Aub*, and *Ago3* could resist the transcription of the TEs[^Brennecke_08]. In germ cells, piRNA maturation occurs in nuage through three pathways; the de **novo biogenesis pathway, the ping-pong cycle (i.e., piRNA amplification pathway), and phasing**[^Sato_20].
...
> Cite: Samanthi, 2022[^Dr.Samanthi]


|![](https://i.pinimg.com/originals/7c/ca/f2/7ccaf212d4ac0a97edba9d057ef2dcae.jpg)|
|:-:|
|[© Samanthi, 2022](https://www.differencebetween.com/what-is-the-difference-between-dna-transposons-and-retrotransposons/)|





[^Dr.Samanthi]:Dr.Samanthi, 2022; What is the Difference Between DNA Transposons and Retrotransposons
...
|![](https://www.researchgate.net/publication/303395236/figure/fig1/AS:1086762969505793@1636115891374/Two-distinct-sized-particles-are-produced-from-a-Nora-virus-infection-of-Drosophila.jpg)|
|:-:|
|Brad Ericson, 2016. Nora Virus from D. melanogaster[^Ericson_16]|

Previous studies show that some **peptidoglycan recognition proteins** and **antimicrobial peptides** from **Toll** and **Imd** pathway may responsible for **sigma virus** infection in *D. melanogaster*[^Carpenter_09][^Tsai_08]. Gene ***ref(2)P***[^Longdon_12][^Magwire_11] which related to selective autophagy, ***CHKov1***, and ***CHKov2***, other resistance factors[^Magwire_12] may work together to increase the resistance of the sigma virus.

DCV can increase the mortality of the flies. ***hopscotch*** mutant makes *D. melanogaster* more vulnerable to DCV which indicates that the Jak-Stat pathway is important to flies against DCV[^Dostert_05][^Kemp_13]. Besides, ***pastrel*** was also connected to DCV mortality[^Magwire_12]. Though the **RNAi mechanism** in DCV defense is unclear, it could potentially play an important role in virus resistance. Similarly, RNAi related mechanism, including ***piwi***, vasa intronic gene (***vig***), aubergine (***aub***), armitage (***armi***), ***Rm62***, ***r2d2*** and Argonaute2 (***AGO2***) as having vital roles in responding to Drosophila X Virus (DXV)[^Zambon_06]. RNAi pathway initiation denpends on the dsRNA produced by genome or RNA viruses.

Others like the **picoran-like Nora virus** could evade the effects of RNAi, Toll, and Jak-STAT pathways[^Habayeb_09]. They studied the genes ***AGO2***, ***Dicer-2***, ***r2d2***, which could help the ***RISC*** assembly[^Liu_Q_03], from siRNA pathway; ***piwi1*** and ***piwi2*** from piRNA pathway, ***Dicer-1*** from miRNA pathway, ***pelle*** from Toll pathway, and ***hop*** from Jak-Stat pathway to show that those 5 commonly involved in viral infection doesn't affect the activity of Nora virus. But flies are able to clean viruses by themselves in low titer infection experiments.
...
General review: [The Drosophila Imd Signaling Pathway; 2014](https://www.jimmunol.org/content/192/8/3455.short)[^Myllymäki_H_14]


*[AMP]: Antimicrobial peptides

!!!note Why Imd is so important:

Imd (mmune deficiency) pathway which could activate 3^rd^ *Drosopila* NFκB protein, Relish[^Dushay_M_96], to activate different kind AMP expression[^Lemaitre_B_95].


The first step for activating Imd signaling pathway is recognizing microbes by Peptidoglycan-recognition proteins (*PGRP*)[^Werner_T_00]. *PGRP* family could be a intracellular, extracellular, or transmembrane proteins.
- PGRP-LD: unclear
- PGRP-LC: transmembrane receptor and binds  a meso-diaminopimelic acid–type PGN found on Gram-negative bacteria and some certain G^+^ Bacteria like Bacillus spp.[^Kaneko_T_04]. It is the principal receptor for activate the Imd signaling pathway[^Choe_K_02].
    - PGRP-LCx: recognizes polymeric PGN[^Kaneko_T_04].
    - PGRP-LCa: Not bind PGN directly, but acts as a coreceptor with PGRP-LCx to bind monomeric PGN fragmetns(tracheal cytotoxin, or TCT)[^Lim_J_06].
- PGRP-LE: cytoplasmic PGRP-LE could interact and activate Imd signaling pathway directly[^Kaneko_T_06].
    - Short from: secreted and binds PGN in the hemolymph[^Takehana_A_02]; It might help to present the PGN to *PGRP-LC*. It can introduce autophagy[^Schmidt_R_08].
    - Full form: in the cytoplasm and recognize TCT fragments[^Lim_J_06].
- ==PGRP-LF==: transmembrane protein, acts as a inhibitor by binds with *PGRP-LC* and prevent its dimerization[^Persson_C_07].
- PGRP-LA: positively regulate the Imd pathway in barrier epithelia[^Gendrin_M_13].
- PGRP-LB, PGRP-SC1A, PGRP-SC1B, and PGRP-SC2 have an amidase activity and are shown to play somewhat redundant roles in downregulating the Imd pathway during a systemic response[^Myllymäki_H_14].

| ![](https://www.jimmunol.org/content/jimmunol/192/8/3455/F1.large.jpg) |
| :-------------: |
| [© Myllymäki H; 2014](https://www.jimmunol.org/content/192/8/3455.short)       |


Imd was cleaved by *Dredd*, which removes 30-aa N-terminal fragment and create a bind site to *lap2*, which can then K63-ubiquitinate Imd[^Meinander_A_12][^Paquette_N_10]. After that, Tab2/Tak1 complex is recruited and Drosophila IKK complex was phosphorylated[^Lu_Y_01][^Kleino_A_05]. At the end of the Imd signaling pathway, *Relish* was phosphorylated on multiple sites at N terminus by *IKK* complex[^Silverman_N_00]. Among them, S528 and S529 phosphorylation are crucial to recruitment RNA PoII[^Silverman_N_00][^Ertürk-Hasdemir_D_09]. C-terminal (Rel-49) remains in cytoplasm and N-terminal (Rel-68) form as a dimer and translocates into nucleus to activate transcription[^Delaney_J_06].

Ubiquiting could also regulates the Imd signaling pathways, too. *dUSP36/Scny* accumulation prevent actviate form, K63-ubiquitinated Imd and promotes its K48-linked ubiquitination and subsequent degradation[^Thevenon_D_09]. Similarly, deubiquitinating enzyme *faf* could prevent Imd ubiquitin[^Yagi_Y_13]. RING-finger protein POSH could degrade *Tak1*[^Tsuda_M_05]. Deubiquitinating *CYLD* could also negatively regulated Imd by interact with IKK[^Tsichritzis_T_07]. Relish activate is relied on degradation by SCF complex[^Khush_R_02] and might promoted by ubiquitin-binding protein dRYBP[^Aparicio_R_13]. Caspar is also inhibits the Dredd-mediated cleavage of Relish[^Kim_M_06].

Transcription factor: *Imd* pathway is negatively regulated by transcriptional factor *zfh1* and the *AP-1/Stat92E* complex. *Zfh1* could down-regulating dramatically in vitro but not quit clear in vivo[^Goto_A_08][^Kim_T_05][^Myllymäki_H_13]. The JNK pathway TF AP-1, Stat92E, and Dsp1 thant bands to the *Relish* target genes.
...
>
> This reducing environment is maintained by the action of antioxidant enzymes and substances, such as superoxide dismutase (**SOD**), catalase (**cat**), glutathione peroxidase (**Gpxl**/**Gtpx**), glutathione (**GSH**), ascorbate (**V~C~**), α-tocopherol ( **V~E~**), and thioredoxin (**Prx4**). Alterations in the redox state and depletion of antioxidants by exposure to oxidants lead to oxidative stress and resultant oxidative injury.
>
> The oxygen molecule (dioxygen; O~2~) qualifies as a radical because (in its ground state) it has two unpaired electrons. From a quantum mechanical standpoint, the electron spin quantum number for ground state dioxygen is three (i.e., oxygen is a triplet species). Since most stable organic molecules are in singlet state (electron spin quantum number equal to one), the reaction of ground state oxygen with cellular constituents is “spin forbidden” even though oxygen is considered a powerful oxidizing agent. To overcome spin restriction, oxygen prefers to accept electrons one at a time, and the sequential addition of electrons leads to formation of ROS. Acceptance of a single electron by an oxygen molecule forms O~2~^·−^, which has one unpaired electron. Superoxide itself has limited reactivity. It is capable of inactivating few enzymes directly. The reduced nicotinamide adenine dinucleotide phosphate dehydrogenase complex of the mitochondrial electron transport chain is one of the enzymes shown to be a direct target for O~2~^·−^ attack[^Zhang_Y_90].
> cite: [Bayr, 2005](https://journals.lww.com/ccmjournal/Fulltext/2005/12001/Reactive_oxygen_species.00031.aspx) (**Bold words** are added by me)
...
!!! infor Source of ROS
    The main source of ROS comes from nicotinamide adenine dinucleotide phosphate (NADPH) oxidase, xanthine oxidoreductase (XOR), and myeloperoxidase (MPO)

- Important function in innate immunity[^C_Dahlgren_99]
- signaling molecules: H~2~O~2~ regulating NFκB, MAPK pathways[^J_Zhang_16]
- It could produced by:
    - Mitochondria[^J_J_Perry_10]
    - NADPH oxidase[^J_L_Meitzler_14] (DUOX)
    -  Peroxisomes (which contain enzymes that produce H~2~O~2~ e.g. polyamine oxidase)[^M_Fransen_12].

Increasing intestine epithelia cells calcium could increase the ROS level and it could be rescude by EGTA or diphenylene iodonium (DPI)[^Ha_E_05].  Some oncogenes activation could increasing ROS[^A_Maya_Mendoza_15][^M_T_Park_14]. ROS oxidize dNTPs which is needed for polymerase[^Y_Meng_18][^D_Graindorge_15]. On the other hand, ROX may slower the replication speed[^K_Somyajit_17], as a physical obstacle[^Y_Sedletska_13] and broken it to form fragments[^P_Kotsantis_18]. *NRF2* deficiency could also increase ROS because *NRF2* TF promotes the expression of oxidative stress related genes[^S_Zhou_13].

Read this one: [ROS and the DNA damage response in cancer](https://www.sciencedirect.com/science/article/pii/S2213231718309017#bib28)
